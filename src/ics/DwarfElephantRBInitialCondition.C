...
template <>
InputParameters
validParams<DwarfElephantRBInitialCondition>()
{
  InputParameters params = validParams<InitialCondition>();
  params.addRequiredParam<UserObjectName>("initial_rb_userobject", "Name of the"
                                          " UserObject for initializing the RB"
                                          " system.");
  params.addParam<unsigned int>("ID_IC_q", 0, "ID of the current intial"
                                "condition vector");
  params.addParam<unsigned int>("ID_IC_q_for_shared_nodes", 0, "ID of the current"
                                "intial condition vector in case of shared nodes.");
  params.addParam<bool>("vector_separation_according_to_subdomains", false,
                        "Tells whether the intial condition vector is separated"
                        " according to the subdomain_ids");
  params.addParam<bool>("shared_node_separation_according_to_subdomains", false,
                        "Tells whether the intial condition vector is separated"
                        " according to the subdomain_ids for the shared nodes");

  return params;
}

DwarfElephantRBInitialCondition::DwarfElephantRBInitialCondition(const InputParameters & parameters)
  ...
  _ID_IC_q(getParam<unsigned int>("ID_IC_q")),
  _ID_IC_q_for_shared_nodes(getParam<unsigned int>("ID_IC_q_for_shared_nodes")),
  _vector_separation_according_to_subdomains(getParam<bool>("vector_separation_according_to_subdomains")),
  _shared_node_separation_according_to_subdomains(getParam<bool>("shared_node_separation_according_to_subdomains"))
{
}
...
void
DwarfElephantRBInitialCondition::compute()
{
  if(_vector_separation_according_to_subdomains)
    _ID_IC_q = _current_elem->subdomain_id() - _ID_first_block;
  ...
  // Lock the new_vector since it is shared among threads.
  {
    ...
    {
      // Set the initial conditions for the RB method
      // Retrieve the node boundary list for obtaining the shared nodes
