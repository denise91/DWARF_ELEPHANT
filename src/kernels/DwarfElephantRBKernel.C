...
DwarfElephantRBKernel::initialSetup()
{
  ...
  if(_simulation_type == "steady")  // SteadyState
  {
    ...
    if(_initialize_rb_system->getExecuteOnEnum().getNames()[1] != _initial)
  ...
  else
  {
    ...
    if(_initialize_rb_system_transient->getExecuteOnEnum().getNames()[1] != _initial)
...
DwarfElephantRBKernel::computeOutput()
{
 if(_vector_seperation_according_to_subdomains)
  ...
  if(_simulation_type == "steady")  // SteadyState
  {
    if(_initialize_rb_system->getOfflineStage())
      // Add the calculated vectors to the vectors from the RB system.
        _initialize_rb_system->getOutputs()[0][_ID_Oq] -> add_vector(_local_out, _var.dofIndices());
  }
  else if (_simulation_type == "transient") // Transient
  {
    if(_initialize_rb_system_transient->getOfflineStage())
