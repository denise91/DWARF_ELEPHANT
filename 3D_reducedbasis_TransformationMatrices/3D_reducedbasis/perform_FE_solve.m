function [] = perform_FE_solve(param_num,mesh_num,test_case)
...
node_coords_file_base = strcat("/home/2014-0004_focal_therapy/PhDs/AdapTT/Nikhil/DwarfElephant/libmesh_mesh_data_opt_ref_geom/mesh_",int2str(mesh_num),"/mesh_node_coords");
node_connect_file_base = strcat("/home/2014-0004_focal_therapy/PhDs/AdapTT/Nikhil/DwarfElephant/libmesh_mesh_data_opt_ref_geom/mesh_",int2str(mesh_num),"/mesh_node_connectivity");
...
load_system_mats_start = tic;
 ...
 addpath(strcat('/home/2014-0004_focal_therapy/PhDs/AdapTT/Nikhil/DwarfElephant/3DRBRFAMatrices/OptRefGeomMatrices/mesh',int2str(mesh_num)));
 ...
 rmpath(strcat('/home/2014-0004_focal_therapy/PhDs/AdapTT/Nikhil/DwarfElephant/3DRBRFAMatrices/OptRefGeomMatrices/mesh',int2str(mesh_num)));
 ...
 addpath(strcat('/home/2014-0004_focal_therapy/PhDs/AdapTT/Nikhil/DwarfElephant/3DRBRFAMatrices/OptRefGeomMatrices/F_vectors/param_',param_label_S(param_num),'/mesh',int2str(mesh_num)));
 ...
 rmpath(strcat('/home/2014-0004_focal_therapy/PhDs/AdapTT/Nikhil/DwarfElephant/3DRBRFAMatrices/OptRefGeomMatrices/F_vectors/param_',param_label_S(param_num),'/mesh',int2str(mesh_num)));
...
get_memory_usage_from_qstat();
 ...
 % Assemble M matrix
  for q_m = 0:Qm %For loop can be parallelized
      %fprintf(1, '[%s] q_m=%d of %d\n', datestr(now, 31), q_m, Qm);
      ...
      if (q_m == 30)
        ...
        %M_mu = M_mu + feval(strcat('RBMTheta_',int2str(q_m)),L,r,r_0,l,l_0,h,d,rho_C_b) * Mq_mat_cell{q_m+1};
      else
        M_mu = M_mu + RBMTheta_handle(L,r,r_0,l,l_0,h,d,rho_C_t) * Mq_mat_cell{q_m+1};
        %M_mu = M_mu + feval(strcat('RBMTheta_',int2str(q_m)),L,r,r_0,l,l_0,h,d,rho_C_t) * Mq_mat_cell{q_m+1};
      end
      inner_prod_mat_L2 = inner_prod_mat_L2 + RBMTheta_handle(L,r,r_0,l,l_0,h,d,1.0)*Mq_mat_cell{q_m+1};
      %[mem,vmem] = get_memory_usage();
      %fprintf(1,"Added M_q %d of %d; Memory consumption: rss=%e Mb; vmem=%e Mb;\n",q_m,Qm,mem - mem_init,vmem - vmem_init);
      %get_memory_usage_from_qstat();
  end
  fprintf(1,'\n');
 ...
 % Assemble A matrix
  for q_a = 0:Qa %For loop can be parallelized
      ...
      if (q_a >= 246) && (q_a <= 248)
        ...
        %A_mu = A_mu + feval(strcat('RBTheta_',int2str(q_a)),L,r,r_0,l,l_0,h,d,k_b) * Aq_mat_cell{q_a+1};
      else
        A_mu = A_mu + RBATheta_handle(L,r,r_0,l,l_0,h,d,k_t) * Aq_mat_cell{q_a+1};
        %A_mu = A_mu + feval(strcat('RBTheta_',int2str(q_a)),L,r,r_0,l,l_0,h,d,k_t) * Aq_mat_cell{q_a+1};
      end
      %[mem,vmem] = get_memory_usage();
      %fprintf(1,"Added A_q %d of %d; Memory consumption: rss=%e Mb; vmem=%e Mb;\n",q_a,Qa,mem - mem_init,vmem - vmem_init);
      %get_memory_usage_from_qstat();
  end
  fprintf(1,'\n');
 % Assemble inner product matrix
  for q_a = 0:Qa %For loop can be parallelized
      ...
      if (q_a >= 246) && (q_a <= 248)
          ...
          %inner_prod_mat = inner_prod_mat + feval(strcat('RBTheta_',int2str(q_a)),L,r,r,l,l,h,d,k_b) * Aq_mat_cell{q_a+1};
      else
          inner_prod_mat = inner_prod_mat + RBATheta_handle(L,r,r,l,l,h,d,k_t) * Aq_mat_cell{q_a+1};
          %inner_prod_mat = inner_prod_mat + feval(strcat('RBTheta_',int2str(q_a)),L,r,r,l,l,h,d,k_t) * Aq_mat_cell{q_a+1};
      end
      %[mem,vmem] = get_memory_usage();
      %fprintf(1,"Inner product matrix. Added A_q %d of %d; Memory consumption: rss=%e Mb; vmem=%e Mb;\n",q_a,Qa,mem - mem_init,vmem - vmem_init);
  end
  fprintf(1,'\n');
 ...
 % Assemble F vector
  for q_f = 0:Qf %For loop can be parallelized
      ...
      %[mem,vmem] = get_memory_usage();
      %fprintf(1,"Added F_q %d of %d; Memory consumption: rss=%e Mb; vmem=%e Mb;\n",q_f,Qf,mem - mem_init,vmem - vmem_init);
  end
 ...
 disp("Solving FE Problem")
  ...
  node_x_new = deform_mesh_fast(node_x, subdomain_ids, element_node, r_0, l_0);
...
% Solve transient problem (backward Euler)
  ...
  for time_step = 2:num_time_steps+1
      x = pcg(A_mu + M_mu/dt,F_mu + M_mu*x_old(:)/dt,1e-10,1000,R_i,R_i');
