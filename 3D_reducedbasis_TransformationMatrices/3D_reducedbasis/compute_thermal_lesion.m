function compute_thermal_lesion(mu_list,RB_Eval,EIM_data,mesh_data)
  ...
  for mu = mu_list
    ...
    [mesh_node_coords_new] = deform_mesh_RB(mesh_data.node_x, mesh_data.subdomain_ids, mesh_data.element_node, mu{1});
    output_unit = fopen(strcat('lesion_data_r_0_',num2str(mu{1}('r_0')),'_l_0_',num2str(mu{1}('l_0')),'_x_',num2str(mu{1}('needle_x')),'_y_',num2str(mu{1}('needle_y')),'_z_',num2str(mu{1}('needle_z')),'_theta_',num2str(mu{1}('needle_theta')),'_phi_',num2str(mu{1}('needle_phi')),'.vtu'),'w');
    ...
    output_unit = fopen(strcat('final_temp_r_0_',num2str(mu{1}('r_0')),'_l_0_',num2str(mu{1}('l_0')),'_x_',num2str(mu{1}('needle_x')),'_y_',num2str(mu{1}('needle_y')),'_z_',num2str(mu{1}('needle_z')),'_theta_',num2str(mu{1}('needle_theta')),'_phi_',num2str(mu{1}('needle_phi')),'.vtu'),'w');
    ...
    fprintf(1,'RB soln computed at:\n r_0 = %e\n l_0 = %e\n needle_x = %e\n needle_y = %e\n needle_z = %e\n needle_theta = %e\n needle_phi = %e\n',mu{1}('r_0'),mu{1}('l_0'),mu{1}('needle_x'),mu{1}('needle_y'),mu{1}('needle_z'),mu{1}('needle_phi'),mu{1}('needle_theta'));
