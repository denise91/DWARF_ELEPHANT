...
for geom_param in r_l_new: 
  for i in range(4):
    offset = i*10
    writeInputFile("Obtain_Mq.i",mesh_file_base.format(r=geom_param[0],l=geom_param[1]),geom_param[0],geom_param[1],offset,mat_file_location.format(param_subd=geom_param[2]),mesh_num)
    subprocess_cmd("qsh; ./DwarfElephant-opt -i Obtain_Mq.i")
 
for geom_param in r_l_new:
  for i in range(32,40): #CHECK PATH GIVEN TO rm COMMAND
    subprocess.call("rm " + mat_file_location.format(param_subd=geom_param[2]) + "mesh" + str(mesh_num) + "/Mq_"+str(i)+".m",shell=True)
