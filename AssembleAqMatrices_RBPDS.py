...
mat_file_location = "/home/2014-0004_focal_therapy/PhDs/AdapTT/Nikhil/DwarfElephant/3DRBRFAMatrices/PDSplit/{param_subd}/"#CHECK if testcase or not
...
for geom_param in r_l_new:
  for i in range(26):
    offset = i*10
    writeInputFile("Obtain_Aq.i",mesh_file_base.format(r=geom_param[0],l=geom_param[1]),geom_param[0],geom_param[1],offset,KernelIDs_and_blocks,mat_file_location.format(param_subd=geom_param[2]),mesh_num)
...
for geom_param in r_l_new:
  for i in range(253,260): # CHECK PATH GIVEN TO rm
    subprocess.call("rm " + mat_file_location.format(param_subd=geom_param[2]) + "mesh" + str(mesh_num) + "/Aq_"+str(i)+".m",shell=True)
