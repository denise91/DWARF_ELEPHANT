...
#include "TestDwarfElephantInitializeRBSystemTransientT3F1O1M1.h"

registerMooseObject("DwarfElephantApp", TestDwarfElephantInitializeRBSystemTransientT3F1O1M1);

template<>
InputParameters validParams<TestDwarfElephantInitializeRBSystemTransientT3F1O1M1>()
{
  InputParameters params = validParams<DwarfElephantInitializeRBSystemTransient>();
    return params;
}

TestDwarfElephantInitializeRBSystemTransientT3F1O1M1::TestDwarfElephantInitializeRBSystemTransientT3F1O1M1(const InputParameters & params):
  DwarfElephantInitializeRBSystemTransient(params)
{
}

void
TestDwarfElephantInitializeRBSystemTransientT3F1O1M1::execute()
{
  if(!_initialized){
  ...
  if (_offline_stage){
    ...
    TestDwarfElephantRBEvaluationTransientT3F1O1M1 _rb_eval(_mesh_ptr->comm(), _fe_problem);
