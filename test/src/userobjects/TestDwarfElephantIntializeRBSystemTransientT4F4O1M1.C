...
#include "TestDwarfElephantInitializeRBSystemTransientT4F4O1M1.h"

registerMooseObject("DwarfElephantApp", TestDwarfElephantInitializeRBSystemTransientT4F4O1M1);

template<>
InputParameters validParams<TestDwarfElephantInitializeRBSystemTransientT4F4O1M1>()
{
  InputParameters params = validParams<DwarfElephantInitializeRBSystemTransient>();
    return params;
}

TestDwarfElephantInitializeRBSystemTransientT4F4O1M1::TestDwarfElephantInitializeRBSystemTransientT4F4O1M1(const InputParameters & params):
  DwarfElephantInitializeRBSystemTransient(params)
{
}

void
TestDwarfElephantInitializeRBSystemTransientT4F4O1M1::execute()
{
  if(!_initialized){
  ...
  if (_offline_stage){
    ...
    TestDwarfElephantRBEvaluationTransientT4F4O1M1 _rb_eval(_mesh_ptr->comm(), _fe_problem);
