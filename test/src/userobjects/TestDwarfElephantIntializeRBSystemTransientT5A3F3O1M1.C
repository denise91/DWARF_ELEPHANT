...
#include "TestDwarfElephantInitializeRBSystemTransientT5A3F3O1M1.h"

registerMooseObject("DwarfElephantApp", TestDwarfElephantInitializeRBSystemTransientT5A3F3O1M1);

template<>
InputParameters validParams<TestDwarfElephantInitializeRBSystemTransientT5A3F3O1M1>()
{
  InputParameters params = validParams<DwarfElephantInitializeRBSystemTransient>();
    return params;
}

TestDwarfElephantInitializeRBSystemTransientT5A3F3O1M1::TestDwarfElephantInitializeRBSystemTransientT5A3F3O1M1(const InputParameters & params):
  DwarfElephantInitializeRBSystemTransient(params)
{
}

void
TestDwarfElephantInitializeRBSystemTransientT5A3F3O1M1::execute()
{
  if(!_initialized){
  ...
  if (_offline_stage){
    ...
    TestDwarfElephantRBEvaluationTransientT5A3F3O1M1 _rb_eval(_mesh_ptr->comm(), _fe_problem);
