...
#include "TestDwarfElephantInitializeRBSystemTransientT3A4F1O1M1.h"

registerMooseObject("DwarfElephantApp", TestDwarfElephantInitializeRBSystemTransientT3A4F1O1M1);

template<>
InputParameters validParams<TestDwarfElephantInitializeRBSystemTransientT3A4F1O1M1>()
{
  InputParameters params = validParams<DwarfElephantInitializeRBSystemTransient>();
    return params;
}

TestDwarfElephantInitializeRBSystemTransientT3A4F1O1M1::TestDwarfElephantInitializeRBSystemTransientT3A4F1O1M1(const InputParameters & params):
  DwarfElephantInitializeRBSystemTransient(params)
{
}

void
TestDwarfElephantInitializeRBSystemTransientT3A4F1O1M1::execute()
{
  if(!_initialized){
  ...
  if (_offline_stage){
    ...
    TestDwarfElephantRBEvaluationTransientT3A4F1O1M1 _rb_eval(_mesh_ptr->comm(), _fe_problem);
